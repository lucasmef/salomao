...
def test_update_account_preserves_c6_secret_material_when_form_is_resubmitted_without_plaintext() -> None:
    session = _build_session()
    try:
        _company, user = _build_company_and_user(session)
        account = create_account(
            AccountCreate(
                ...
                c6_certificate_pem="---CERT---",
                c6_private_key_pem="---KEY---",
            ),
            db=session,
            current_user=user,
        )
        ...
        updated = update_account(
            account.id,
            AccountCreate(
                name="Conta C6 Atualizada",
                account_type="checking",
                bank_code="336",
                account_number="123456",
                c6_api_enabled=True,
                c6_environment="sandbox",
                c6_client_id="client-id-2",
                c6_partner_software_name="Gestor Financeiro ERP",
        ...
        assert updated.c6_client_id == "client-id-2"
        assert updated.c6_partner_software_name == "Gestor Financeiro ERP"
        assert updated.c6_partner_software_version == "0.2.0"
        assert updated.has_c6_client_secret is True
        assert updated.has_c6_certificate is True
        assert updated.has_c6_private_key is True
    finally:
        session.close()


def test_enabling_c6_api_disables_other_accounts_from_same_company() -> None:
    session = _build_session()
    try:
        _company, user = _build_company_and_user(session)
        first = create_account(
            AccountCreate(
                ...
                account_type="checking",
                c6_api_enabled=True,
                c6_client_id="client-1",
                c6_partner_software_name="Gestor Financeiro",
                c6_partner_software_version="0.1.0",
                c6_client_secret="secret-1",
                c6_certificate_pem="---CERT-1---",
                c6_private_key_pem="---KEY-1---",
            ),
            db=session,
            current_user=user,
        )
        second = create_account(
            AccountCreate(
                ...
                account_type="checking",
                c6_api_enabled=True,
                c6_client_id="client-2",
                c6_partner_software_name="Gestor Financeiro",
                c6_partner_software_version="0.1.0",
                c6_client_secret="secret-2",
                c6_certificate_pem="---CERT-2---",
                c6_private_key_pem="---KEY-2---",
            ),
            db=session,
            current_user=user,
        )
        ...
        assert persisted_first is not None
        assert persisted_second is not None
        assert persisted_first.c6_api_enabled is False
        assert persisted_second.c6_api_enabled is True
    finally:
        session.close()

...
from app.db.models.linx import LinxCustomer, ReceivableTitle
...
def test_issue_inter_charges_uses_linx_customer_registration_for_new_client() -> None:
    session = _build_session()
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.path == "/cobranca/v3/cobrancas" and request.method == "POST":
            ...
            created_payloads.append(payload)
            return httpx.Response(200, json={"codigoSolicitacao": "SOL-LINX-1"})
        if request.url.path == "/cobranca/v3/cobrancas/SOL-LINX-1":
            return httpx.Response(
                200,
                json={
                    "cobranca": {
                        "codigoSolicitacao": "SOL-LINX-1",
                        "seuNumero": "12345",
                        "situacao": "A_RECEBER",
                        "dataEmissao": "2026-03-01",
                        "dataVencimento": "2026-03-12",
                        "valorNominal": "250.00",
                        "valorTotalRecebido": "0.00",
                        "pagador": {"nome": "Cristini Serafim Nandi", "cpfCnpj": "12345678901"},
                    },
                    "boleto": {"codigoBarras": "111222333444", "linhaDigitavel": "11122.233344"},
                },
            )
        raise AssertionError(f"Requisicao inesperada: {request.url}")
    ...
    try:
        ...
        session.add(
            BoletoCustomerConfig(
                company_id=company.id,
                client_key=normalize_text("Cristini Serafim Nandi"),
                client_name="Cristini Serafim Nandi",
                uses_boleto=True,
                mode="individual",
            )
        )
        session.add(
            LinxCustomer(
                company_id=company.id,
                linx_code=1001,
                display_name="Cristini Serafim Nandi",
                legal_name="Cristini Serafim Nandi",
        ...
        session.add(
            ReceivableTitle(
                ...
                original_amount=Decimal("250.00"),
                amount_with_interest=Decimal("250.00"),
                customer_name="Cristini Serafim Nandi",
                document_reference="DOC-1",
                status="Em aberto",
            )
        )
        ...
        assert result.message == "Boletos emitidos no Inter com sucesso."
        assert created_payloads
        assert created_payloads[0]["pagador"] == {
            "tipoPessoa": "FISICA",
            "nome": "Cristini Serafim Nandi",
            "cpfCnpj": "12345678901",
